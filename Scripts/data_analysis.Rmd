...
There was a second round of extra speceis requested on July of 2026, these are:

From Peru and Chile EEZs
- Anchoveta (Engraulis ringens) *Done in previous round*
- Spanish sardine (Sardinops sagax)
- Inca scad/Chilean jack mackerel (Trachurus murphyi)
- Common sardine (Strangomera bentincki) *Note: name in DBEM is wrong, it apears as "Clupea bentincki" Araucanian herring so be cautious about this result.*
...
Norway, Denmark, UK EEZs
- Atlantic cod (Gadus morhua)

Norway, Denmark EEZs
- European sprat (Sprattus sprattus)
- N Atlantic herring (Clupea harengus)
- Atlantic mackerel (Scomber scombrus) *Done in previous round*
- Blue whiting (Micromesistius poutassou)

Mexico EEZ
- Pacific threadfin herring (Opisthonema libertate)
...
spp_selected <- c(
  ...
  # ------------------- NEW REQUESTED SPECIES 
  "Sardinops sagax",           # Spanish sardine
  "Trachurus murphyi",         # Inca scad / Chilean jack mackerel
  "Clupea bentincki",     # Common sardine 
  ...
  "Clupea harengus",           # N Atlantic herring
  "Micromesistius poutassou",  # Blue whiting
  "Opisthonema libertate",     # Pacific threadfin herring
...
spp_df <- data.frame(
  taxon_name = c(
    rep("Sardinops sagax", 2),	
    rep("Trachurus murphyi", 2),
    rep("Clupea bentincki", 2),
    rep("Scomber japonicus", 2),
    rep("Brevoortia tyrannus", 5),
    rep("Oncorhynchus kisutch", 5),
    rep("Oncorhynchus gorbuscha", 5),
    rep("Oncorhynchus nerka", 5),
    rep("Gadus morhua", 4),
    rep("Sprattus sprattus", 3),
    rep("Clupea harengus", 3),
    rep("Micromesistius poutassou", 3),
    rep("Opisthonema libertate", 1),
    rep("Merluccius productus", 3)
  ),
  common_name = c(
    rep("Spanish sardine", 2),
    rep("Chilean jack mackerel", 2),
    rep("Common sardine", 2),
    rep("Pacific chub mackerel", 2),
    rep("Atlantic menhaden", 5),
    rep("Coho salmon", 5),
    rep("Pink salmon", 5),
    rep("Sockeye salmon", 5),
    rep("Atlantic cod", 4),
    rep("European sprat", 3),
    rep("N Atlantic herring", 3),
    rep("Blue whiting", 3),
    rep("Pacific threadfin herring", 1),
    rep("North Pacific hake", 3)
  ),
  region = c(
    "Chile",
    "Peru",
...
spp_list <- spp_list %>% 
  left_join(spp_df) %>% 
  mutate(common_name = ifelse(taxon_key == 601530,"Common sardine",common_name))
...
mainfx <- function(taxon){
  ...
  scenrios <- list.files("/Volumes/Enterprise/Data/dbem/dbem_update",full.names = T)
  
  to_read <- paste0(taxon,"Catch.RData")
  
  
  # Filter species list to the taxon of interest
  spp_df_region <- spp_list %>% 
    filter(taxon_key %in% taxon)
  
  # filter the regional grid to the EEZs of interest
  regional_grid <- my_data("sau_index") %>% 
    filter(eez_name %in% spp_df_region$region)
  ...
  for(m in 1:6){
    ...
    if(file.exists(file_to_read)){
      ...
      future_data <-  spp_data %>% 
        ...
        mutate(
          esm = str_sub(file_to_read,45,48),
          ssp = ifelse(str_sub(file_to_read,49,50) == 26,"ssp126","ssp585")
        )
  ...
  spp_name <- spp_list %>% filter(taxon_key %in% taxon) %>% pull(common_name) %>% unique()
...
suppressWarnings(
  lapply(par_spp_list[9:14], mainfx)
)
```
