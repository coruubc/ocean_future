...
min_value <- round(min(all_2050$change,na.rm = T))
max_value <- ceiling(max(all_2050$change,na.rm = T))
map_name <- paste0("all_hotspots_map.png")
...
all_map <- 
  ...
  scale_fill_viridis_b("% Change by 2030 Relative to Historical Value", 
                       ...
                       breaks = seq(min_value,max_value,10),
...
wc_pacific_line <- wc_pacific %>% 
  mutate(
    ymax = ifelse(per_change_rm+sd_per_change > 100,100,per_change_rm+sd_per_change),
    ymin = ifelse(per_change_rm-sd_per_change < -100, -100, per_change_rm-sd_per_change),
    eez_name = ifelse(str_detect(eez_name,"Kiribati"),"Kiribati",
                      ifelse(str_detect(eez_name,"Tokelau"),"Tokelau",
                             ifelse(str_detect(eez_name,"Micronesia"),"Micronesia",
                                           eez_name
                                    )
                             )
                      )
  ...
  my_ggtheme_p(ax_tx_s = 7,ax_tl_s = 10,leg_tl_s = 10,leg_tx_s = 10,leg_pos = "",facet_tx_s = "8",x_vjust = 0.5,axx_tx_ang = 45) +
  labs(
    x= "Year",
    y = "% Change Relative to Historical Value\n(All species aggregated)"
  ) +
  facet_grid(ssp~eez_name,
             labeller = labeller(ssp = c("126" = "SSP1-2.6",
  "585" = "SSP5-8.5")))

line_name <- paste0(selected_hotspot$hotspot,"_lines_facet.png")[1]
...
selected_hotspot <- hotspot_data %>% 
  filter(hotspot == "France")

print(unique(selected_hotspot$name))

```

[1] "Corsica (France)"        "France (Mediterranean)"  "France (Atlantic Coast)"



```{r france_data, eval = T, echo = T}
...
print(france_sf$name)
length(unique(france_sf$name))
...
min_value <- round(min(france_2050$change,na.rm = T))
max_value <- ceiling(max(france_2050$change,na.rm = T))


min_x <- floor(st_bbox(france_sf)[1])
max_x <- ceiling(st_bbox(france_sf)[3])
min_y <- floor(st_bbox(france_sf)[2])
max_y <- ceiling(st_bbox(france_sf)[4])


france_map <- 
  france_sf %>% 
  left_join(france_2050,
            by = "name") %>% 
  # st_shift_longitude() %>% 
  ggplot() +
  my_land_map(fill = "grey80") +
  geom_sf(aes(fill = change)) +
  # geom_sf_label(aes(label = name, fill = change), size = 1.5, color = "grey50") +
    ggrepel::geom_label_repel(data = . %>% filter(name %in% c("France (Atlantic Coast)")),
                              aes(label = name, fill = change, geometry = geometry),
                              stat = "sf_coordinates",
                              min.segment.length = 1,
                              size = 2, color = "white") +
  ggrepel::geom_label_repel(data = . %>% filter(!name %in% c("France (Atlantic Coast)")),
                            aes(label = name, fill = change, geometry = geometry),
                              ...
                              size = 2, color = "black") +
  scale_fill_viridis_b("% Change by 2030 Relative to Historical Value", 
                       limits = c(min_value,max_value),breaks = seq(min_value,max_value,2),
                           guide = guide_colorbar(title.position = "right", title.hjust = 0.5, barwidth = 1, barheight = 15)
                       ) +
  my_ggtheme_p(ax_tx_s = 9,ax_tl_s = 10,leg_tl_s = 10,leg_tx_s = 10,leg_pos = "right",facet_tx_s = "10") +
  theme(legend.title = element_text(angle = 270, vjust = 0.5, hjust = 0.5))+
...
france_panel <-
  ggdraw() +
  draw_plot(france_map, x = 0, y = .50, height = .50, width = 1) +
  draw_plot(france_line, x = 0, y = 0, height = .51, width = 1) +
  draw_label('A', x = 0.002, y = .99, hjust = 0, vjust = 1, 
             size = 10) +
  draw_label('B', x = 0.002, y = .52, hjust = 0, vjust = 1, 
             size = 10)
...
length(unique(somalia$eez_name))
...
print(somalia_sf$name)
length(unique(somalia_sf$name))
...
min_value <- round(min(somalia_2050$change,na.rm = T))
max_value <- ceiling(max(somalia_2050$change,na.rm = T))


min_x <- floor(st_bbox(somalia_sf)[1])
max_x <- ceiling(st_bbox(somalia_sf)[3])
min_y <- floor(st_bbox(somalia_sf)[2])
max_y <- ceiling(st_bbox(somalia_sf)[4])


somalia_map <- 
  somalia_sf %>% 
  left_join(somalia_2050,
            by = "name") %>% 
  # st_shift_longitude() %>% 
  ggplot() +
  my_land_map(fill = "grey80") +
  ...
  scale_fill_viridis_b("% Change by 2030 Relative to Historical Value", 
                       limits = c(min_value,max_value),breaks = seq(min_value,max_value,2),
                           guide = guide_colorbar(title.position = "right", title.hjust = 0.5)
                       ) +
  my_ggtheme_p(ax_tx_s = 9,ax_tl_s = 10,leg_tl_s = 10,leg_tx_s = 10,leg_pos = "right",facet_tx_s = "10") +
  theme(legend.title = element_text(angle = 270, vjust = 0.5, hjust = 0.5))+
  labs(
...
somalia_panel <-
  ggdraw() +
  draw_plot(somalia_map, x = 0, y = .50, height = .50, width = 1) +
  draw_plot(somalia_line, x = 0, y = 0, height = .51, width = 1) +
  draw_label('A', x = 0.002, y = .99, hjust = 0, vjust = 1, 
             size = 10) +
  draw_label('B', x = 0.002, y = .52, hjust = 0, vjust = 1, 
             size = 10)
...
print(poland_sf$name)
length(unique(poland_sf$name))
...
min_value <- round(min(poland_2050$change,na.rm = T))
max_value <- ceiling(max(poland_2050$change,na.rm = T))


min_x <- floor(st_bbox(poland_sf)[1])
max_x <- ceiling(st_bbox(poland_sf)[3])
min_y <- floor(st_bbox(poland_sf)[2])
max_y <- ceiling(st_bbox(poland_sf)[4])


poland_map <- 
  poland_sf %>% 
  left_join(poland_2050,
            by = "name") %>% 
  # st_shift_longitude() %>% 
  ggplot() +
  my_land_map(fill = "grey80") +
...
poland_panel <-
  ggdraw() +
  draw_plot(poland_map, x = 0, y = .50, height = .50, width = 1) +
  draw_plot(poland_line, x = 0, y = 0, height = .51, width = 1) +
  draw_label('A', x = 0.002, y = .99, hjust = 0, vjust = 1, 
             size = 10) +
  draw_label('B', x = 0.002, y = .52, hjust = 0, vjust = 1, 
             size = 10)
...
```{r mozambique_data, eval = T, echo = T}

# Oceans future data
mozambique <- future_data %>% 
  filter(
    eez_name %in% selected_hotspot$eez_nam
  )

length(unique(mozambique$eez_name))

# SAU shapefile data
mozambique_sf <- sau_sf %>% 
  filter(
    name %in% selected_hotspot$name
  )

print(mozambique_sf$name)
length(unique(mozambique_sf$name))
...
min_value <- round(min(mozambique_2050$change,na.rm = T))
max_value <- ceiling(max(mozambique_2050$change,na.rm = T))


min_x <- floor(st_bbox(mozambique_sf)[1])
max_x <- ceiling(st_bbox(mozambique_sf)[3])
min_y <- floor(st_bbox(mozambique_sf)[2])
max_y <- ceiling(st_bbox(mozambique_sf)[4])


mozambique_map <- 
  mozambique_sf %>% 
  left_join(mozambique_2050,
            by = "name") %>% 
  # st_shift_longitude() %>% 
  ggplot() +
  my_land_map(fill = "grey80") +
...
mozambique_panel <-
  ggdraw() +
  draw_plot(mozambique_map, x = 0, y = .50, height = .50, width = 1) +
  draw_plot(mozambique_line, x = 0, y = 0, height = .51, width = 1) +
  draw_label('A', x = 0.002, y = .99, hjust = 0, vjust = 1, 
             size = 10) +
  draw_label('B', x = 0.002, y = .52, hjust = 0, vjust = 1, 
             size = 10)
...
selected_hotspot <- hotspot_data %>% 
  filter(hotspot == "Svalbard")

print(unique(selected_hotspot$name))

```

[1] "Svalbard Isl. (Norway)"


```{r svalbard_data, eval = T, echo = T}

# Oceans future data
svalbard <- future_data %>% 
  filter(
    eez_name %in% selected_hotspot$eez_nam
  )

length(unique(svalbard$eez_name))
...
print(svalbard_sf$name)
length(unique(svalbard_sf$name))
...
svalbard_line <- svalbard %>% 
  ...
  scale_y_continuous(limits =c(-30,100),
                     breaks = seq(-30,100,15),
                     labels = c(seq(-30,75,15),"> 90")
                     # labels = c("< -100",seq(-75,75,25),"100")
...
min_value <- round(min(svalbard_2050$change,na.rm = T))
max_value <- ceiling(max(svalbard_2050$change,na.rm = T))


min_x <- floor(st_bbox(svalbard_sf)[1])
max_x <- ceiling(st_bbox(svalbard_sf)[3])
min_y <- floor(st_bbox(svalbard_sf)[2])
max_y <- ceiling(st_bbox(svalbard_sf)[4])


svalbard_map <- 
  svalbard_sf %>% 
  left_join(svalbard_2050,
            by = "name") %>% 
  # st_shift_longitude() %>% 
  ggplot() +
  my_land_map(fill = "grey80") +
...
svalbard_panel <-
  ggdraw() +
  draw_plot(svalbard_map, x = 0, y = .50, height = .50, width = 1) +
  draw_plot(svalbard_line, x = 0, y = 0, height = .51, width = 1) +
  draw_label('A', x = 0.002, y = .99, hjust = 0, vjust = 1, 
             size = 10) +
  draw_label('B', x = 0.002, y = .52, hjust = 0, vjust = 1, 
             size = 10)
...
min_value <- round(min(equ_2050$change,na.rm = T))
max_value <- ceiling(max(equ_2050$change,na.rm = T))


min_x <- floor(st_bbox(equ_sf)[1])
max_x <- ceiling(st_bbox(equ_sf)[3])
min_y <- floor(st_bbox(equ_sf)[2])
max_y <- ceiling(st_bbox(equ_sf)[4])
...
equ_panel <-
  ggdraw() +
  draw_plot(equ_map, x = 0, y = .50, height = .50, width = 1) +
  draw_plot(equ_line, x = 0, y = 0, height = .51, width = 1) +
  draw_label('A', x = 0.002, y = .99, hjust = 0, vjust = 1, 
             size = 10) +
  draw_label('B', x = 0.002, y = .52, hjust = 0, vjust = 1, 
             size = 10)
...
```{r can_arct_data, eval = T, echo = T}

# Oceans future data
can_arct <- future_data %>% 
  filter(
    eez_name %in% selected_hotspot$eez_nam
  )

length(unique(can_arct$eez_name))
...
print(can_arct_sf$name)
length(unique(can_arct_sf$name))
...
can_arct_line <- can_arct %>% 
  mutate(
    per_change_rm = ifelse(per_change_rm > 100,100,per_change_rm),
    ymax = ifelse(per_change_rm+sd_per_change > 100,100,per_change_rm+sd_per_change),
    ymin = ifelse(per_change_rm-sd_per_change < -100, -100, per_change_rm-sd_per_change)
  ) %>% 
  # View()
...
min_value <- round(min(can_arct_2050$change,na.rm = T))
max_value <- ceiling(max(can_arct_2050$change,na.rm = T))


min_x <- floor(st_bbox(can_arct_sf)[1])
max_x <- ceiling(st_bbox(can_arct_sf)[3])
min_y <- floor(st_bbox(can_arct_sf)[2])
max_y <- ceiling(st_bbox(can_arct_sf)[4])


can_arct_map <- 
  can_arct_sf %>% 
  left_join(can_arct_2050,
            by = "name") %>% 
  # st_shift_longitude() %>% 
  ggplot() +
  my_land_map(fill = "grey80") +
  ...
  scale_fill_viridis_b("% Change by 2030 Relative to Historical Value", 
                       limits = c(min_value,max_value),breaks = seq(min_value,max_value,1),
                           guide = guide_colorbar(title.position = "right", title.hjust = 0.5,barheight = 15)
                       ) +
  my_ggtheme_p(ax_tx_s = 9,ax_tl_s = 10,leg_tl_s = 10,leg_tx_s = 10,leg_pos = "right",facet_tx_s = "10") +
  theme(legend.title = element_text(angle = 270, vjust = 0.5, hjust = 0.5))+
  labs(
...
can_arct_panel <-
  ggdraw() +
  draw_plot(can_arct_map, x = 0, y = .50, height = .50, width = 1) +
  draw_plot(can_arct_line, x = 0, y = 0, height = .51, width = 1) +
  draw_label('A', x = 0.002, y = .99, hjust = 0, vjust = 1, 
             size = 10) +
  draw_label('B', x = 0.002, y = .52, hjust = 0, vjust = 1, 
             size = 10)
...
selected_hotspot <- hotspot_data %>% 
  filter(hotspot == "Mexico (Atlantic)")
...
min_value <- round(min(mex_2050$change,na.rm = T))
max_value <- ceiling(max(mex_2050$change,na.rm = T))


min_x <- floor(st_bbox(mex_sf)[1])
max_x <- ceiling(st_bbox(mex_sf)[3])
min_y <- floor(st_bbox(mex_sf)[2])
max_y <- ceiling(st_bbox(mex_sf)[4])
...
mex_map <- 
  ...
  scale_fill_viridis_b("% Change by 2030 Relative to Historical Value", 
                       limits = c(min_value,max_value),breaks = seq(min_value,max_value,1),
                           guide = guide_colorbar(title.position = "right", title.hjust = 0.5,barheight = 5)
                       ) +
  my_ggtheme_p(ax_tx_s = 9,ax_tl_s = 10,leg_tl_s = 10,leg_tx_s = 10,leg_pos = "right",facet_tx_s = "10") +
  theme(legend.title = element_text(angle = 270, vjust = 0.5, hjust = 0.5))+
  labs(
...
mex_panel <-
  ggdraw() +
  draw_plot(mex_map, x = 0, y = .50, height = .50, width = 1) +
  draw_plot(mex_line, x = 0, y = 0, height = .51, width = 1) +
  draw_label('A', x = 0.002, y = .99, hjust = 0, vjust = 1, 
             size = 10) +
  draw_label('B', x = 0.002, y = .52, hjust = 0, vjust = 1, 
             size = 10)
...
selected_hotspot <- hotspot_data %>% 
  filter(hotspot == "Russia")

print(unique(selected_hotspot$name))

```

[1] "Russia (Kara Sea)" "Russia (Far East)"


```{r russia_data, eval = T, echo = T}
...
print(russia_sf$name)
length(unique(russia_sf$name))
...
min_value <- round(min(russia_2050$change,na.rm = T))
max_value <- ceiling(max(russia_2050$change,na.rm = T))


min_x <- floor(st_bbox(russia_sf)[1])
max_x <- ceiling(st_bbox(russia_sf)[3])
min_y <- floor(st_bbox(russia_sf)[2])
max_y <- ceiling(st_bbox(russia_sf)[4])


russia_map <- 
  russia_sf %>% 
  left_join(russia_2050,
            by = "name") %>% 
  st_shift_longitude() %>%
  ggplot() +
  my_land_map(fill = "grey80") +
  geom_sf(aes(fill = change)) +
  scale_fill_viridis_b("% Change by 2030 Relative to Historical Value", 
                       limits = c(min_value,40),
                       breaks = seq(min_value,40,10),
                           guide = guide_colorbar(title.position = "right", title.hjust = 0.5,barheight = 15)
                       ) +
  my_ggtheme_p(ax_tx_s = 9,ax_tl_s = 10,leg_tl_s = 10,leg_tx_s = 10,leg_pos = "right",facet_tx_s = "10") +
  theme(legend.title = element_text(angle = 270, vjust = 0.5, hjust = 0.5))+
...
russia_panel <-
  ggdraw() +
  draw_plot(russia_map, x = 0, y = .50, height = .50, width = 1) +
  draw_plot(russia_line, x = 0, y = 0, height = .51, width = 1) +
  draw_label('A', x = 0.002, y = .99, hjust = 0, vjust = 1, 
             size = 10) +
  draw_label('B', x = 0.002, y = .52, hjust = 0, vjust = 1, 
             size = 10)
