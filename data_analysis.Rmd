...
spp_selected <- "Engraulis ringens"
    ...
    if(file.exists(file_to_read)){
      ...
      future_data <-
        ...
        mutate(
          ...
          species = spp_selected,
          eez_id = ifelse(eez_id == eez_name)
        ) %>% 
        group_by(species,eez_id) %>% 
        mutate(
        ...
        select(species,eez_id,year,per_change_rm)
...
# Get DBEM coords
dbem_coords <- read_csv("~/Library/CloudStorage/OneDrive-UBC/Data/Spatial/DBEM/Lon_Lat_DBEM.txt",
                        col_names = FALSE)
colnames(dbem_coords) <- c("index","lon","lat")
...
# Get the DBEM path
files_path <- list.files(my_path("G","dbem/dbem_cmip6/r_data/C6GFDL85F1_t2"),
                         full.names = T)
